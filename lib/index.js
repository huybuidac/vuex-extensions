...
var createStore = function createStore(vuexStoreClass) {
  ...
  if (!vuexStoreClass.prototype.reset) {
    ...
    vuexStoreClass.prototype.reset = function (options) {
      var originalState = getOriginalState(this._modules.root, options);
...
function getOriginalState(module) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var defaultReset = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  ...
  var state = (options.self ? module._rawModule._originalState : module.state) || {};
